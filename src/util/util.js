 ...
 * @returns 
 */
export const getAllParentNodeByChildNode =(arr, pid,keyName)=> {   
    var temp = [];
    var callback = function (nowArr, pid) {     //先定义个函数寻找子节点位置 找到后 再找改节点父元素位置 以此类推
        ...
        var item = nowArr[i];
            if (item[keyName] === pid) {
                ...
                if (item.children) {
                    callback(item.children, pid); //menus 子节点字段名称
                }
            }
        }
    };
